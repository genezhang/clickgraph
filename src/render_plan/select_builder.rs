                            ...
                            match self.get_properties_with_table_alias(&table_alias.0) {
                                Ok((properties, actual_table_alias_opt)) => {
                                    if properties.is_empty() {
                                        // Check if this is a path variable (e.g., RETURN p where p is from MATCH p = ...)
                                        // Path variables don't have properties like nodes do - they have path columns
                                        // from the VLP CTE: path_nodes, path_edges, path_relationships, hop_count
                                        ...
                                        // SCALAR FIX: If no properties found, treat as a scalar column alias
                                        // This handles cases like:
                                        //   WITH n.email as group_key ... RETURN group_key
                                        // where 'group_key' is a scalar (String from email), not a node
                                        // Using ColumnAlias prevents wildcard expansion (group_key.*)
                                        // which is invalid for scalars in ClickHouse
                                        log::info!(
                                            "🔍 Treating '{}' as scalar ColumnAlias (no node properties found)",
                                            table_alias.0
                                        );
                                        select_items.push(SelectItem {
