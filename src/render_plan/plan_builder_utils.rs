...
// has no caller left here so it is not re-exported). Sub-slice C: the
// join-condition rewrite group (all called only internally in this module).
pub(crate) use super::cte_rewrite::{
    collect_with_cte_table_aliases, remap_cte_names_in_render_plan,
    rewrite_join_conditions_for_cte_aliases, rewrite_operator_application_for_cte,
    rewrite_operator_application_for_cte_join, strip_table_alias_from_resolved,
};

// P2.5 sub-slice D (REFACTORING_SAFETY_PLAN.md §5.1): the LogicalPlan-level
// CTE-reference rewriters moved to `cte_graph_joins_rewrite.rs`. Only
// `update_graph_joins_cte_refs` has in-module callers (2 sites), so it is the
// only one re-exported; the other four are called solely within that module.
pub(crate) use super::cte_graph_joins_rewrite::update_graph_joins_cte_refs;
                                ...
                                bridge_join.table_alias);
                                        ...
                                        (cte_col, pattern_alias, pattern_col)
                                    {
                                        ...
                                        if let Some(pidx) = pattern_join_idx {
                                            ...
                                            with_cte_render.joins.0.retain(|j| {
                                                j.table_alias != bridge_join.table_alias
                                            });
                                        ...
                                        matches!(j.join_type, super::JoinType::Left)
                                            ...
                                            if let Some(pidx2) = person_join_idx {
                                                let person_alias = with_cte_render.joins.0[pidx2]
                                                    .table_alias
                                                    .clone();
                                                ...
                                                if !person_still_needed {
                                                    ...
                                                    let mut fk_info: Option<(String, String)> =
                                                        None; // (rel_alias, fk_col)
                                                    for op in
                                                        &with_cte_render.joins.0[pidx2].joining_on
                                                    {
                                                        for operand in &op.operands {
                                    ...
                                    ) = (cte_col, pattern_alias.clone(), pattern_col)
                                    {
                                        ...
                                        let edge_to_old_from = with_cte_render
                                            ...
                                            .and_then(|j| {
                                                j.from_id_column.clone().or(j.to_id_column.clone())
                                            });
...
// P2.6 slice 1 (REFACTORING_SAFETY_PLAN.md §5.1): the #529 shape-1 loud-guard
// property helpers (`table_role_dependent_property_names`,
// `collect_property_accesses`) + their unit tests moved verbatim to the new
// `render_plan::with_to_cte` module (the WITH→CTE "entangled core"). Re-exported
// here `pub(crate)` during the transition so `build_chained_with_match_cte_plan`'s
// call sites keep resolving unqualified. Both were widened `fn` → `pub(crate) fn`
// at the move site (the sole non-verbatim change); bodies are byte-identical.
pub(crate) use super::with_to_cte::{
    collect_property_accesses, table_role_dependent_property_names,
};

// P2.6 slice 3 (REFACTORING_SAFETY_PLAN.md §5.1): the WITH-discovery / join-
// pruning cluster (`find_all_with_clauses_grouped`, `collapse_passthrough_with`,
// `node_is_concrete_labeled`, `alias_has_pattern_correlation`,
// `prune_joins_covered_by_cte`) moved verbatim to the `render_plan::with_to_cte`
// module. The four names with callers still here (`build_chained_with_match_cte_plan`
// + the `#[cfg(test)]` characterization tests below) are re-exported during the
// transition; `alias_has_pattern_correlation` was widened `fn` → `pub(crate) fn`
// at the move site to allow it. `node_is_concrete_labeled` stays private in
// `with_to_cte` (its only caller, `alias_has_pattern_correlation`, moved with
// it). Bodies are byte-identical.
pub(crate) use super::with_to_cte::{
    alias_has_pattern_correlation, collapse_passthrough_with, find_all_with_clauses_grouped,
    prune_joins_covered_by_cte,
};

// P2.6 slice 2 (REFACTORING_SAFETY_PLAN.md §5.1): the giant WITH→CTE-reference
// rewriter `replace_with_clause_with_cte_reference_v2` moved verbatim to the
// `render_plan::with_to_cte` module. Re-exported here `pub(crate)` during the
// transition so `build_chained_with_match_cte_plan`'s call site keeps resolving
// unqualified. No visibility change was needed (already `pub(crate)`); body is
// byte-identical.
pub(crate) use super::with_to_cte::replace_with_clause_with_cte_reference_v2;
...
// transition so the existing callers on this path keep resolving.
