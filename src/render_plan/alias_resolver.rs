...
impl AliasResolverContext {
    ...
    fn transform_expr(&self, expr: LogicalExpr) -> LogicalExpr {
        match expr {
            ...
            LogicalExpr::List(list) => {
                LogicalExpr::List(list.into_iter().map(|e| self.transform_expr(e)).collect())
            }
    ...
    fn register_standard(&mut self, cypher_alias: String, table_alias: String) {
        self.resolutions
            .insert(cypher_alias, AliasResolution::StandardTable { table_alias });
    }
    ...
    fn register_denormalized(
        ...
        let actual_alias =
            if let Some(unified_alias) = self.coupled_edge_aliases.get(&relationship_alias) {
                log::debug!(
                    "Coupled edge: Using unified alias {} instead of {} for node {}",
                    unified_alias,
                    relationship_alias,
                    node_alias
                );
        ...
        for (table_name, entries) in edge_info {
            if entries.len() > 1 {
                ...
                for (alias, _) in entries.iter().skip(1) {
                    self.coupled_edge_aliases
                        .insert(alias.clone(), first_alias.clone());
                }
    ...
    fn collect_edge_tables_with_labels(
        &self,
        plan: &LogicalPlan,
        edge_info: &mut HashMap<String, Vec<(String, Option<Vec<String>>)>>,
    ) {
        match plan {
            LogicalPlan::GraphRel(rel) => {
    ...
    fn analyze_plan(&mut self, plan: &LogicalPlan) {
        match plan {
            ...
            LogicalPlan::GraphRel(rel) => {
                ...
                let (has_from_props, has_to_props) =
                    if let LogicalPlan::ViewScan(scan) = rel.center.as_ref() {
                        (
