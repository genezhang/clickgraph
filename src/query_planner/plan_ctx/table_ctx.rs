...
impl TableCtx {
    ...
    pub fn is_path_variable(&self) -> bool {
        ...
        // Exclude anonymous node aliases (t1, t2, t3, ...)
        // These are generated by generate_id() and have pattern "t\d+"
        if self.alias.len() >= 2 && self.alias.starts_with('t') {
            // Check if rest is all digits
            if self.alias[1..].chars().all(|c| c.is_ascii_digit()) {
                return false; // It's an anonymous node alias, not a path variable
            }
        }
