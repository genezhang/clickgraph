    ...
    fn analyze_node(plan: &LogicalPlan, requirements: &mut PropertyRequirements) {
        match plan {
            ...
            LogicalPlan::Projection(projection) => {
                log::info!(
                    "🔍 PropertyRequirementsAnalyzer: Analyzing RETURN projection with {} items",
            ...
            LogicalPlan::Union(union) => {
                log::info!(
                    "🔍 PropertyRequirementsAnalyzer: Analyzing Union with {} branches",
                    union.inputs.len()
                );
            ...
            LogicalPlan::Unwind(unwind) => {
                log::info!(
                    "🔍 PropertyRequirementsAnalyzer: Analyzing UNWIND (alias: {})",
                    unwind.alias
                );
                ...
                if is_wildcard {
                    log::info!(
                        "🔍 UNWIND alias '{}' requires ALL properties (wildcard)",
                        unwind.alias
                    );
                } else if let Some(ref props) = specific_props {
                    log::info!(
                        "🔍 UNWIND alias '{}' has requirements: {:?}",
                        unwind.alias,
                        props
                    );
                } else {
                    log::info!(
                        "🔍 UNWIND alias '{}' has no downstream requirements",
                        unwind.alias
                    );
                }
                ...
                if let LogicalExpr::AggregateFnCall(agg) = &unwind.expression {
                    if agg.name.to_lowercase() == "collect" && agg.args.len() == 1 {
                        if let LogicalExpr::TableAlias(ref source_alias) = agg.args[0] {
                            ...
                            if is_wildcard {
                                log::info!(
                                    "🔍 Source '{}' requires ALL properties (wildcard)",
                                    source
                                );
    ...
    fn analyze(
        ...
        for alias in requirements.aliases() {
            if requirements.requires_all(alias) {
                log::info!(
                    "  📋 {}: ALL properties (wildcard or whole node return)",
                    alias
                );
...
mod tests {
    ...
    fn test_unwind_property_mapping() {
        ...
        assert!(
            f_props.contains("name"),
            "Expected 'name' property for alias 'f', got: {:?}",
            f_props
        );
    }
