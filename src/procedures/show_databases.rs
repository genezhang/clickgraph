...
pub fn execute(_schema: &GraphSchema) -> Result<Vec<HashMap<String, Value>>, String> {
    ...
    if let Some(schemas_lock) = crate::server::GLOBAL_SCHEMAS.get() {
        ...
        for (name, _) in schemas_map.iter() {
            ...
            record.insert("access".to_string(), Value::String("read-write".to_string()));
            record.insert("address".to_string(), Value::String("localhost:7687".to_string()));
            record.insert("role".to_string(), Value::String("primary".to_string()));
            record.insert("writer".to_string(), Value::Bool(true));
            record.insert("requestedStatus".to_string(), Value::String("online".to_string()));
            record.insert("currentStatus".to_string(), Value::String("online".to_string()));
