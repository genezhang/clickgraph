...
use crate::query_planner::analyzer::multi_type_vlp_expansion::{enumerate_vlp_paths, PathEnumeration};
...
impl<'a> MultiTypeVlpJoinGenerator<'a> {
    ...
    pub fn add_property(&mut self, cypher_name: String, type_mappings: HashMap<String, (String, String)>) {
    ...
    fn generate_path_branch_sql(&self, path: &PathEnumeration, branch_idx: usize) -> Result<String, String> {
        let hops = &path.hops;
        if hops.is_empty() {
        ...
        if let Some(ref filters) = self.start_filters {
            ...
            let start_filter = filters
                ...
                .replace(&format!("{}.", self.start_alias), &format!("{}.", start_alias_sql));
        ...
        for (hop_idx, hop) in hops.iter().enumerate() {
            ...
            if is_fk_edge {
                ...
                let join_sql = format!(
                    "INNER JOIN {} {} ON {}.{} = {}.{}",
                    rel_table, end_node_alias, current_alias, start_id_col, end_node_alias, from_col
                );
        ...
        if let Some(ref filters) = self.end_filters {
            let end_filter = filters
                ...
                .replace(&format!("{}.", self.end_alias), &format!("{}.", end_node_alias));
    ...
    fn generate_select_items(&self, node_alias: &str, node_type: &str, start_alias_sql: &str) -> Vec<String> {
        ...
        if let Ok(node_schema) = self.schema.get_nodes_schemas().get(node_type).ok_or("Node not found") {
            let end_id_sql = if node_schema.node_id.is_composite() {
                ...
                let cols: Vec<String> = node_schema.node_id.columns()
                    .iter()
                ...
                format!("toString({}.{}) AS end_id", node_alias, node_schema.node_id.column())
            };
        ...
        if let Ok(start_type) = self.start_labels.first().ok_or("No start type") {
            if let Ok(node_schema) = self.schema.get_nodes_schemas().get(start_type).ok_or("Node not found") {
                let start_id_sql = if node_schema.node_id.is_composite() {
                    ...
                    format!("toString({}.{}) AS start_id", start_alias_sql, node_schema.node_id.column())
                };
...
mod tests {
    ...
    fn test_property_projection_structure() {
        ...
        type_mappings.insert("User".to_string(), ("full_name".to_string(), "u".to_string()));
