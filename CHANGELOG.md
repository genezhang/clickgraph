...
- **Stale `xfail` markers in `test_graphrag_multi_type.py` following #557** (#570): 9 tests were marked `xfail(reason="Code bug: multi-type VLP generates invalid SQL or crashes server")`. Re-ran against a live ClickGraph server (ClickHouse-backed) after #557's multi-type VLP unlabeled-end-node CTE-count-mismatch fix: 6 of the 9 now genuinely pass (`test_follows_or_authored_one_to_two_hops`, `test_multi_type_exact_paths`, `test_length_with_multi_type`, `test_relationships_with_multi_type`, `test_multi_type_all_users`, `test_multi_type_vlp_different_properties`) and had their stale `xfail` markers removed. The remaining 3 (`test_multi_type_two_hops_only`, `test_json_extraction_sql_generation`, `test_cte_columns_direct_access`) still genuinely fail and keep their markers; two unrelated `xfail`s (missing-property schema-validation gap) were left untouched.
