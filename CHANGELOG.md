...
- **DeltaGraph: Databricks SQL Warehouse support** (PRs #316–#338): A second SQL dialect alongside ClickHouse, plumbed through the existing renderer and executor stack so Cypher can target Databricks/Spark without forking the engine. Phased rollout:
  ...
  Still pending before a Databricks GA: `MERGE`, full LDBC validation against a live warehouse, OAuth M2M auth, external-link result chunks, and the `catalog:` YAML field in `GraphSchema` (Phase 3.2 — currently lives in `DatabricksConfig` only). Plan: [`docs/design/DELTAGRAPH_PLAN.md`](docs/design/DELTAGRAPH_PLAN.md).
