...
- **#1085 — a variable-length-path endpoint named `t` silently returned wrong rows for a chained hop; now loud-gated.** ClickGraph's VLP CTE uses a fixed reserved outer FROM alias (`VLP_CTE_FROM_ALIAS = "t"`), threaded as a string-level protocol through the render/emit phases (endpoint join resolution, `t.start_*`/`t.end_*` column references, the reverse-branch `swap_vlp_start_end` text substitution). When a user names a VLP endpoint exactly `t`, that reserved alias becomes indistinguishable from the user's node: the endpoint rewriters re-resolve an already-correct `t.end_id` back to `t.start_id`, so a chained hop after the path (`(t)-[:R*1..2]->(t2)-[:DISCUSSES]->(d)`) anchors on the VLP **start** instead of the endpoint and returns the wrong documents (those of the start, not the reachable endpoints) — a silent, plausible-looking wrong result. It only became reachable once #1081 made the shape plan. The complete fix (making the reserved alias query-scoped) is the multi-VLP-aliasing work already tracked under `TODO(multi-vlp)`; until that lands, the colliding pattern is **rejected loudly** at plan time (`pre_register_vlp_endpoints`) with an actionable error ("rename the `t` variable") rather than emitted as silently-wrong SQL (ground rule 1). The bug is entirely the alias collision — **any** other endpoint alias resolves correctly (the single-statement `-[:R*1..2]->(t2)-[:DISCUSSES]->(d)` fusion works verbatim, join key `t.end_id`, endpoint property projected), verified live on ClickHouse. Byte-identical for the whole corpus (the gate only errors on a `t`-aliased VLP endpoint; no golden used that shape). Locked by `cs_standard_1085_vlp_endpoint_named_t_is_loud`.
...
- **S1 stats-informed anchor selection, flag-gated** (P-5, `docs/design/STATS_PLANNING.md`): new `graph_catalog::table_stats` module — `TableStatsSnapshot` (immutable `db.table → total_rows` map), pluggable async `TableStatsSource` (ClickHouse implementation: one `SELECT database, name, total_rows FROM system.tables WHERE database IN (…)`; NULL `total_rows` = unknown, never 0), and a TTL-refreshed `TableStatsCache` (`CLICKGRAPH_STATS_TTL_SECS`, default 300; lazy refresh on access, stale-serving + no-hammer on fetch failure). Enabled only by `CLICKGRAPH_STATS_ENABLED=true` (default **off**) in remote ClickHouse server mode: startup installs the cache, the HTTP/Bolt query handlers attach a snapshot to the task-local `QueryContext` at request entry, and `select_anchor()` ranks anchor candidates **within each existing priority tier** by ascending row count (unknown counts last, alphabetical among themselves) before the historical alphabetical tie-break. Ordering only — never row membership (PRIORITIES.md §1.7); with the flag off (and in all sql_only/embedded/test paths, which never attach a snapshot) generated SQL is byte-identical to the stats-less engine, locked by the untouched golden + corpus suites plus an explicit empty-snapshot degradation test. A new separate with-stats golden set (`tests/rust/integration/stats_anchor_golden_tests.rs`, `golden/sql_ir/stats_standard/`) locks the flag-on plan against a fixed programmatic fixture. Embedded/remote library-mode wiring and column-level selectivity (S2) / metrics feedback (S3) are documented follow-ups.

### 🧹 Infrastructure

- **Package the `deltagraph` (Databricks) server for distribution alongside `clickgraph`**: the DeltaGraph server binary was previously built nowhere in CI — neither the Docker image nor the release tarballs shipped it, so a DeltaGraph user had to build from source. It is now packaged for the preview via the model documented in `docs/deltagraph/PACKAGING.md`: (1) the published `genezhang/clickgraph` Docker image is built with `--features databricks` and now contains **both** server binaries — `clickgraph` (ClickHouse, the default `ENTRYPOINT`) and `deltagraph` (Databricks, selected at runtime with `docker run --entrypoint /usr/local/bin/deltagraph …`); (2) `release.yaml` gains a separate `deltagraph` build+package+upload pass on every matrix target (Linux/Windows/macOS-x64/macOS-arm64), attaching `deltagraph-<platform>.tar.gz`/`.zip` to each release. The `databricks` feature is chdb-free (verified: the actual build graph pulls no `chdb`/`libchdb`, neither binary links it, both are ~24 MB), so `deltagraph` cross-compiles to all four targets exactly like `clickgraph`, and the `clickgraph` release tarball stays feature-free and byte-for-byte unchanged (its download carries no Databricks code). A user chooses the backend at **runtime** (pick the binary, or `clickgraph --databricks`) — not at install time. New docs: `docs/deltagraph/PACKAGING.md` (build/selection reference) and `docs/deltagraph/DOCKER_QUICKSTART.md` (no-build Docker walkthrough incl. the Neo4j Browser demo on both backends); the build-from-source `QUICKSTART.md` and README DeltaGraph section link to both. Locally verified: `docker build` compiles both binaries, and the image's `deltagraph` entrypoint answers a Bolt query against a live Databricks free-tier warehouse.
...
- **`toString()` on a whole-valued float dropped the decimal (`toString(3.0)` → `"3"`, not Neo4j's `"3.0"`) (#1055)**: Neo4j retains the Integer/Float distinction in the string form (`toString(3.0)` = `"3.0"`, `toString(3)` = `"3"`, `toString(3.5)` = `"3.5"`), but ClickGraph rendered `"3"` for every whole-valued float — a silent-wrong observable string value. Two roots converge: a whole-float literal already renders as an integer SQL literal (`3.0` → `3`), AND ClickHouse `toString(toFloat64(3))` itself returns `"3"`. Both are cured at one seam by gating on the render-site type classifier: when `toString`'s argument is provably `RenderType::Float` (a float literal, `toFloat(...)`, a declared Float column, or a float-promoting arithmetic expression), it routes to the new dialect method `FunctionMapper::to_string_float` (CH: `if(match(toString(x), '^-?[0-9]+$'), concat(toString(x), '.0'), toString(x))` — appends `.0` iff the string form is a bare integer, so fractional/scientific/NULL/inf/nan pass through untouched; Databricks: `string(double(x))` — Spark keeps a double's decimal). An `Integer`/`Number`/unknown argument keeps the plain `toString` (conservative-None), so `toString(3)`, `toString(abs(x))`, and `toString(size(...))` are byte-identical to before. Landed in BOTH render paths — the final-SELECT emitter (`to_sql_query.rs`) and the CTE-build-stage renderer (`cte_extraction::render_expr_to_sql_string`, "Path C", which backs VLP-bound / pattern-comprehension WHERE filters), mirroring the #880/#871 two-renderer pattern. Also refined `infer_operator_type`'s arithmetic arm to Cypher's numeric promotion (any Float operand → Float result; `^` always Float), which fixes the common negative-whole-float case (`-2.0` lowers to `0 - 2.0`, now `"-2.0"`) and `toString(2 ^ 3)` → `"8.0"` — every other classifier consumer keys on String/Boolean/Date or the `is_numeric()` umbrella (Float ⊆ numeric), so the Number→Float refinement is transparent to them (ratchet-neutral, no baseline bump). Live-verified on `db_standard` over HTTP **and** Bolt (the Neo4j Browser path), both dialects. Locked by 10 goldens (literal / `toFloat` / fractional / negative-whole / exponentiation positive cases + integer / abs-Number / size-Int negative controls + a VLP-WHERE Path-C case, ×2 dialects), a `to_string_float` mapper unit test, and an `arithmetic_promotes_to_float` type-inference unit test.

 for a denormalized self-loop node (`from_node == to_node` sharing the edge table), the swap-branch re-key logic (`cte_extraction.rs`) maps each role's property to the opposite role's canonical key; a property present in only ONE direction's map (`from_node_properties` XOR `to_node_properties`) has no opposite-role key, so on reverse (swapped-orientation) rows it silently resolved to empty — the same class of silent-empty #1027 fixed for the symmetric case. The config validator (`validate_denormalized_nodes`, `graph_catalog/config.rs`) previously rejected only an *entirely-missing* direction, not an asymmetric key *set*, leaving such a schema admissible. It now compares the two role maps' key sets for a denorm self-loop node and rejects a mismatch at schema-load with an error naming the offending properties (ground rule #1: fail loud over silent-wrong). Additive, schema-load only — no SQL-emitting code touched (ratchet-neutral, zero golden churn). Full-repo schema sweep found exactly one asymmetric self-loop denorm schema — `schemas/examples/ontime_denormalized_mismatched.yaml` (`state` only on the destination role) — which is **live-verified silently-wrong** today (an undirected `MATCH (a:Airport)-[:FLIGHT]-(b:Airport) RETURN a.state` renders `a.state` from `DestState` in both union arms, attributing the destination's state to the origin node), so rejecting it is correct per ground rule #1; that example is relabeled as a negative fixture. The one other asymmetric node in the repo (`schemas/examples/zeek_merged.yaml`, Domain, `resolved_ips` only on the to-role) is a **distinct-label** edge (`IP → Domain`, never a self-loop) — the swap-branch re-key hazard is self-loop-only, so the guard correctly does not touch it. Locked by `test_denormalized_selfloop_asymmetric_property_keys_rejected_1038` + `test_denormalized_selfloop_symmetric_property_keys_ok_1038` (symmetric, only mapped columns differ by role → still passes; guards against over-rejection) + `test_ontime_mismatched_example_selfloop_rejected_1038` (locks the real on-disk fixture).

- **Numeric aggregate over a pattern comprehension folded over the constant `1`, not the projected value (#1053)**: `min`/`max`/`sum`/`avg` applied to a pattern comprehension — e.g. `min([(u)-[:FOLLOWS]->(f:User) | f.user_id])` — rendered `MIN(1)`/`MAX(1)`/`SUM(1)`/`AVG(1)` (aggregating the constant literal `1`) instead of `MIN(target_prop)` etc. over the projected `f.user_id`, returning a meaningless, data-independent result for every input row (e.g. `min` always returned `1`). The sibling `collect` arm in the same `match` already folded over `target_prop`; the four numeric arms did not. Fix mirrors the `collect` arm — the numeric arms now emit `<AGG>(target_prop)` when a target JOIN was emitted (`target_join_info.is_some()`), gated identically so the `(1)` cardinality form is kept ONLY when no target value is resolvable. `MIN`/`MAX`/`SUM`/`AVG` are ANSI-standard and byte-identical across ClickHouse and Databricks, so the fix carries no dialect-specific tokens (ratchet-neutral). Byte-additive — corpus sweep zero churn (10 new goldens, no existing golden moved). Live-verified on `db_standard` (`min([… | f.user_id])`: `2` over `{2,3}` vs the buggy `1`; `max`=`3`, `sum`=`5`, `avg`=`2.5`), both dialects. Locked by `pattern_comp_agg_{min,max,sum,avg}_target_prop_1053` + `pattern_comp_agg_min_unlabeled_stays_one_1053`. **Two documented residuals, NOT closed here** (design-cycle, filed on #1053): (1) an **unlabeled** target (`(f)` with no `:User`) has no resolvable target node → `target_join_info == None` → the aggregate keeps `MIN(1)` (needs per-branch label inference, #989/#927 endpoint-resolution territory; the residual guard golden locks this byte-unchanged); (2) empty-comprehension `min`/`max`/`avg` still coalesce to `0` rather than Cypher's `null` — the `coalesce(result, 0)` default is defeated by ClickHouse `join_use_nulls=0` (a LEFT-JOIN miss fills the non-nullable aggregate column with the type-default `0`, so `coalesce(0, null)` = `0`) AND spelling `null` would make ClickHouse (yields `0`) and Databricks (yields `null` on the miss) disagree — the correct fix needs dialect-aware LEFT-JOIN null handling, out of scope for this bounded slice.

- **Bare-disconnected multi-clause undirected OPTIONAL fell back to the legacy two-arm split (#1046, #583 follow-up)**: a pattern of two independent `MATCH` clauses with no shared variable followed by an undirected single-hop OPTIONAL whose anchor is the pre-bound variable — e.g. `MATCH (c:User) MATCH (a:User) OPTIONAL MATCH (a)-[:FOLLOWS]-(b:User) RETURN a.name, b.name` — was not covered by the #583 doubled-edge fix and rendered the spurious-NULL two-arm UNION split (`(anchor, NULL)` + phantom `(NULL, neighbor)` rows; on the standard `social_benchmark` schema the second arm re-declared the anchor alias → a hard ClickHouse **Code 179** `MULTIPLE_EXPRESSIONS_FOR_ALIAS` crash). Root cause: the pre-bound anchor's `CartesianProduct` binding subtree is absorbed into the OPTIONAL hop's `left` (near anchor) or `right` (far anchor) endpoint — a THIRD plan shape the standard-stage scope core `undirected_standard_single_hop_optional_core` (`bidirectional_union.rs`) rejected, because it required BOTH endpoints to be bare `GraphNode`s. Fix loosens that endpoint gate to also accept a `CartesianProduct` on either side (a nested `GraphRel` is still a genuine chain and stays with the #589 loud gate); the render swap seam (`collect_graph_rels`) already walked both `CartesianProduct` branches, so no render change was needed. Scoped to a SINGLE undirected OPTIONAL hop: when a second optional hop is chained onto it over the same cartesian spine, the normalize pass now bails on the whole tree (`has_cartesian_optional_nested_undirected_edge` guard) rather than partially doubling only the first hop — that chained-disconnected shape (which slips past #589's shared-node detector) stays a documented pre-existing residual, unchanged. Byte-additive — corpus sweep unchanged (only the previously-buggy single-hop shapes move), ratchet neutral. Live-verified on `social` (correct 2 rows vs. base Code 179 crash), both dialects. Locked by 4 goldens: `standard_bare_disconnected_undirected_optional_{doubled,far_anchor_doubled,composite_edge_doubled}_1046` + `standard_bare_disconnected_chained_undirected_optional_not_doubled_1046`.

- **Denormalized closed `*0..N` VLP silently dropped every real cycle (#978/#980, #887 Phase 2b)**: `DenormalizedCteStrategy::uses_edge_uniqueness` required `min_hops >= 1`, so `(a)-[:R*0..]->(a)` on a denormalized schema fell back to NODE-uniqueness — which structurally cannot return to the start — and only the zero-length self rows survived (the #978/#980 fail-louds, formerly `UnsupportedFeature` errors, guarded a silent-wrong state that Phase 2b instead fixes). The strategy now mirrors the standard #628 fix: a closed zero-hop pattern (same Cypher variable on both endpoints, `left_node_alias == right_node_alias`) is edge-unique, the zero-hop base seeds an empty `path_edges`, and both fail-louds are deleted. **Live verification exposed a latent shared defect**: a bare `[]` seeds ClickHouse `Array(Nothing)`, which the recursive CTE engine rejects against the recursive arm's `Array(Tuple(...))` (`CANNOT_CONVERT_TYPE`) — the standard #628 zero-hop path had the identical latent bug (never live-hit because `users`/`follows` were empty). Both standard and denormalized zero-hop bases now seed a **typed-empty** `path_edges` via the new shared `typed_empty_edges_seed` (`variable_length_cte.rs`): `(SELECT arraySlice([tuple(__seed_edge.<identity>)], 1, 0) FROM <edge source> AS __seed_edge LIMIT 1)` — a dialect-routed scalar subquery over the edge table, the only source of the real element dtypes (CH `arraySlice`/`tuple`/`[…]`, Spark `slice`/`struct`/`array(…)`). Live-verified against hand-computed trail oracles: denorm `*1..` closed = 12, `*0..` closed = 17 (LAX 5, SFO/JFK/ORD/ATL 3); standard 3-cycle fixture `*1..` = 3, `*0..` = 6. Locked by `denorm_closed_zero_hop_vlp_uses_edge_uniqueness_628_mirror` + updated `closed_zero_hop_vlp_uses_edge_uniqueness_628`, flipped #978/#980 golden tests, and 2 renamed + 8 regenerated corpus goldens.

- **Flat mixed-access (foreign-embedded) endpoint property resolved through the edge table → Code 47** (#1006): on shapes like `(a:Person)-[:REPORTS_TO*1..1|*1|]->(b)` where the FROM endpoint's edge-embedded map is a strict subset of its property set (`from_node_properties: {pid: mgr_id}` only, node's own table `people` carrying `name`), `RETURN a.name` (and `WHERE`/`GROUP BY`/`ORDER BY` on `a.name`) fell through to the edge-alias pass-through and emitted `tN.name` on `reports` — a column that does not exist there → ClickHouse Code 47 (unknown identifier). Fix: select/group-by/order-by builders now register a lazy own-table join request when a property is absent from the edge's embedded map but resolvable from the node's OWN table, and `join_builder` injects a DEDUPLICATED, row-preserving `LEFT JOIN (SELECT <id>, any(<prop>) AS <prop> FROM <own_table> GROUP BY <id>) AS <node_alias>` (the #908 `*_own` recipe, now reached via `FunctionMapper::any()` — CH `any` / Databricks `any_value`); aliased by the NODE alias so SELECT/WHERE/GROUP BY/ORDER BY resolve without rewrites. The filter/order-by/having/group-by path mirrors it in `apply_property_mapping_to_expr` (previously remapped the alias onto the edge table). Exclusions: `node_id` names never resolve through the own table (identity mappings are auto-generated even for VIRTUAL ids whose own table has no such column — e.g. zeek_dns `node_id: ip_address` on `dns_log`; those keep the legacy edge-alias pass-through byte-identical, locked by the corpus) and `__denorm_scan_{alias}` anchors keep their #582/#590 anchor-CTE path (`is_denorm_scan_anchor_alias` guard). 0 corpus golden churn beyond lock-in; the VLP `*1..2` recursive-CTE path (which already used `start_own`) is byte-identical. Locked by `issue_1006_own_table_property_tests` (12 cases: flat/*1/*1..1 SELECT, WHERE, GROUP BY, ORDER BY, embedded-id stays on edge, virtual-id exclusion, VLP CTE unchanged, join dedup, no-join-when-unused).

- **#1006 follow-up: the same own-table join now fires for the FILTER path (WHERE/ORDER BY/GROUP BY/HAVING) and for UNLABELED queries** (same issue): the first slice's registration lived in the select/group-by/order-by builders, so (a) a WHERE-only reference (`MATCH (a:Person)-[:REPORTS_TO]->(b) WHERE a.name = 'Alice' RETURN a.pid, b.name`) was property-mapped with an empty registry and fell through to the denormalized-alias remap → `t1.name` on `reports` (Code 47), and (b) an ORDER BY-only reference (`RETURN a.pid, b.name ORDER BY a.name`) was mapped BEFORE join injection could see it (the ORDER BY node wraps the GraphJoins, so the join-builder's pre-injection walk of `graph_joins.input` never reaches it; the fix registers wrapper-level expressions in `to_render_plan_with_ctx`'s GraphJoins branch before `extract_joins`, plus a plan walker over Filter/GraphRel/OrderBy/GroupBy/having at every registration point — idempotent). Unlabeled queries (`MATCH (a)-[r]->(b)`) additionally left `GraphNode.label` None and `rel.labels` empty, defeating `get_node_label_for_alias` AND the #551/#560 connection-node fallback (rel-type lookup); the new shared `own_table_label_for_alias` adds a schema-wide match of the endpoint's embedded-id KEY (`pid` in `pid: mgr_id`) against node schemas whose own table resolves the property — unique match required, ambiguity bails to the pre-fix fall-through. The VLP guard is unchanged (`*1..1`/`*1` endpoints keep their CTE/`start_own` path; `vlp_range_keeps_cte_path_unchanged` locks it). Locked by 6 new regression cases (WHERE-only plain arrow, ORDER BY-only plain arrow, and unlabeled WHERE/SELECT/ORDER BY variants).

- **Flaky golden: JOIN emission order flapped with the process-global alias counter** (#626): `sort_joins_by_dependency` (the emitter's topological JOIN sort) broke ties among simultaneously-ready joins by **string** alias order, but generated `t{N}` aliases embed the process-global `ALIAS_COUNTER` value — the same logical plan gets `t0/t1` in one process and `t9/t10` in another, and string order flips across the digit-length boundary (`"t10" < "t9"` but `"t11" < "t12"`). So JOIN emission order inside the #492 Incoming-swapped undirected UNION branches (both edge joins simultaneously ready off the FROM node) depended on how many aliases earlier queries in the process had allocated: `partial_ref_undirected_2hop__clickhouse` flapped ~50% in full `cargo test` runs and failed 8/8 in isolation with an unlucky counter phase. Both tie-break sites (ready-set pick and cycle-break fallback) now compare aliases with a trailing-decimal-suffix-aware natural order (`natural_alias_ord`, `t9 < t10`), which follows allocation order = plan order regardless of the counter's absolute value. Semantics unchanged — only the choice among equally-valid ready joins is affected; dependency order still governs. Locked by a unit test reproducing the exact FROM-`b`/`t9`-vs-`t10` shape (verified to fail on the old code) and an Ord-property test; the golden now passes 8/8 isolated runs byte-identical, with full suite + corpus sweep green. Adversarial review: 0 real issues, 0 blocking; Ord contract verified empirically over adversarial alias sets (leading zeros, all-digit, >u64 suffixes). Follow-ups noted, not fixed here: CTE-**name** string sorts (`plan_builder_utils.rs:11144/:11868`) have the same latent `_cte_9`/`_cte_10` boundary flap shape (unreachable until a query allocates ≥10 WITH-CTEs), and `plan_optimizer.rs::topo_sort_joins` is a second topo sorter with a different (input-order) tie-break — unify in the SQL-IR refactor.

- **Undirected VLP (≥2 hops) silently under-counted 40–60%: the two-monotone-arm strategy is replaced by a single directed walk over a doubled-edge set** (#617; subsumes #606's undirected entry): Cypher lets EACH hop of `(a)-[:R*min..max]-(b)` traverse its relationship in either direction independently, but the engine split undirected VLP into a `BidirectionalUnion` of two monotone directed arms — every hop forward (`vlp_a_b`) ∪ every hop backward (`vlp_b_a`, roles swapped) — so any mixed-direction path (`a→m←b`) was structurally unrepresentable. Live counts matched the two-arm model exactly (all missing rows were mixed-direction): trail-semantics oracle over the 10-edge social benchmark: `*2..2` 38→**64**, `*1..2` 58→**84**, `*2..3` 92→**226**, `*3..3` 54→**162**, `*1..3` 112→**246** — per-pair multisets verified row-for-row, both routing paths (flat exact-bound chain AND recursive range CTE) were affected. **Fix**: the `BidirectionalUnion` analyzer pass now NORMALIZES in-scope undirected VLP GraphRels (single known type, same-label endpoints, plain standard/polymorphic edge table via new catalog API `RelationshipSchema::is_plain_edge_table()`, scalar from/to ids, non-shortestPath, not `*1..1`) to `direction: Outgoing` + `was_undirected` in a whole-tree pre-pass (mixed patterns still split their remaining plain undirected hops), instead of splitting them; the render layer re-derives the decision through the ONE shared scope predicate (`undirected_vlp_single_walk_core` — the legacy split's arms also carry `was_undirected`, so the flag alone must never gate codegen) and walks a **doubled-edge sibling CTE** (`undir_edges_{a}_{b}`: each physical edge emitted in BOTH orientations under the original from/to column names — joins unchanged — plus `__cg_orig_from/__cg_orig_to` original-identity columns; every column reference table-qualified, since ClickHouse resolves an unqualified `follower_id` against the reverse arm's OWN `followed_id AS follower_id` alias and silently flips the identity — caught live as symmetric extra+missing rows). Trail-uniqueness compares original-orientation identity everywhere: `path_edges` tuples in the recursive walk, pairwise `NOT (r_i = r_j)` guards in the flat exact chain, and from/to components of a schema-defined composite `edge_id` are remapped to the identity columns (`edge_identity_column` — the polymorphic schema's `(from_id,to_id,type,ts)` edge_id was the live counter-example: unmapped, every orientation looked distinct and range counts degraded to pure walks, 104 vs 84). **Collateral fixes**: OPTIONAL undirected range VLP now works and NULL-extends correctly (main: Code 47; now oracle-exact per-anchor counts with all 8 anchors), `*0..N` no longer double-counts every zero-hop row (both arms used to seed hop 0; `*0..2` = 80, exactly the node-unique-parity oracle), WITH-barrier aggregation over undirected VLP works (was #620-family Code 47), and Cypher UNION with undirected VLP in both arms executes. **Unchanged (byte-identical SQL vs main, verified)**: directed VLP, single/multi-hop non-VLP undirected, `*1..1` (complete under two arms), undirected shortestPath (own BFS/two-arm machinery — its mixed-direction incompleteness is pre-existing and tracked separately), FK-edge (hard-rejected upstream), denormalized, composite-id. Zero ratchet-baseline bumps: schema-pattern classification and the doubled-edge column enumeration (`doubled_edge_passthrough_columns`) live in the schema catalog; the generator gate consumes the threaded `undirected_single_walk` flag rather than re-branching on raw schema flags. Golden drift: 26 files, all undirected-VLP corpus entries (two-arm → single-walk; the aggregation set also loses its per-arm GROUP-BY-over-union scaffolding), plus 14 new #617 goldens (range/exact/zero-hop/optional/endpoint-filter/poly-edge-id/one-hop-unchanged, both dialects). Known-remaining, main-identical, filed as follow-ups: chained continuation off an exact-bound (flat) undirected VLP endpoint still drops the continuation join (pre-existing #602 family — the range/CTE path chains correctly); the VLP-endpoint + chained-pattern + aggregate shape double-joins the continuation node table (pre-existing on directed, undirected now inherits directed's behavior); `length(p)` on exact-bound flat VLP hardcodes 1 (pre-existing on directed); single-hop undirected + UNWIND + WITH silently drops the whole pattern (`FROM system.one`, pre-existing).

  **Adversarial review: two rounds. Round 1 (BLOCK) found 1 genuinely new silent-corruption defect + 3 must-address items, all fixed**: (B1) the doubled-edge CTE name was keyed on endpoint ALIASES only — two Cypher-UNION arms with the same aliases but different relationship types collided on one name, and the arm-merge rename fix-up patches `render.from` but not `joins[].table_name`, so the second arm silently walked the FIRST arm's edges ({2,2} instead of {2,0} on a two-type schema; loud Code 47 when the tables' column names differ). Fixed by appending the sanitized edge table to the name — same name now implies same table, hence same edge set, making keep-one dedup semantics-preserving (reviewer re-verified all three variants live: two-type {2,0}, different-columns {2,2} exact, same-body single definition). (F2) the flat exact-bound doubled CTE ignored `rel_table_parameterized` (server-mode parameterized views would error loud; the range path was already safe) — body FROM now honors it, the NAME stays parameter-free. (F3/F12) new catalog method `RelationshipSchema::doubled_edge_walk_compatible()` excludes schemas that map a property onto the from/to id columns (a per-hop filter on such a property would read the orientation-swapped value) and reserved `__cg_orig_*` column-name collisions — both stay on the legacy two-arm path; the reviewer swept every in-repo schema and confirmed NO real schema loses coverage to the new gate. Round 2: MERGE, no new silent-wrongness; the strategy held against scope-predicate drift (structurally impossible — labels/pattern_combinations finalized before the pass), split-arm leakage into doubled codegen (provably unreachable), `*1..10`=6882/`*`=1442 oracle counts, self-loops/duplicate-edge conventions (pre-existing parity), dotted/quoted columns, and Databricks dialect emission. Review also surfaced (pre-existing, filed): OPTIONAL VLP anchor-WHERE gate silently dropped for ANY VLP (`optional_anchor_gate_conjuncts` bails on `variable_length` — directed identical on main; the undirected variant converts loud Code 53 → silently-wrong), closed-pattern `(a)-[*..]-(a)` constraint dropped engine-wide, `*0..0` renders a nonsense 1-hop join, and `EXISTS((...*...))` panics.

- **Undirected VLP with an endpoint filter (and any recursive-CTE query in a later UNION arm) failed with Code 60** (#609): ClickHouse's analyzer (verified on 25.8) cannot resolve a recursive CTE referenced from the SECOND (or later) arm of a **bare top-level UNION** — `WITH RECURSIVE v1 AS (...), v2 AS (...) SELECT ... FROM v1 UNION ALL SELECT ... FROM v2` errors with `Unknown table expression identifier 'v2'` (arm-swap moves the error to whichever recursive CTE the second arm references; per-arm subquery wrapping does NOT help; wrapping the whole union does). Root-caused by executing bisected SQL variants directly against ClickHouse — the generated SQL was internally consistent all along, and the issue's original "inner-wrapper rename" hypothesis was wrong. Undirected VLP (BidirectionalUnion → two recursive CTEs + top-level UNION ALL) hit this exactly when the query had no ORDER BY / LIMIT / aggregate, each of which already forced the subquery wrapper — which is why the failure looked intermittent (the filed repro fails; add ORDER BY and it "works"). Fixed in `to_sql_query.rs` at both union emission sites: `needs_subquery` now also fires when any CTE `is_recursive` (planner-internal unions), and the Cypher-union per-arm path wraps its output in `SELECT * FROM (...) AS __cypher_union` under the same condition. Live-verified (social benchmark, hand-checked counts): `*1..2` undirected with id/name endpoint filters (2 / 1 rows), both-endpoint, `*0..2`, exact-bound, OPTIONAL undirected, undirected shortestPath, Cypher UNION with VLP arm (8 rows); directed VLP counts byte-stable vs the #598 oracle (29/46), zero-hop #610 shape 18. Golden drift: exactly 2 (undirected shortestPath corpus pair — a previously latent Code 60 of the same class, now wrapped). Follow-ups filed: **#617** (undirected VLP ≥2 hops silently under-counts 40–60% — the two-arm BidirectionalUnion cannot represent mixed-direction paths at all; oracle `*2..2` = 64 true vs 38 engine — strategy-level rework, subsumes #606's undirected entry), **#618** (Cypher UNION with a VLP in BOTH arms: duplicate CTE definition, Code 179, reference renamed but not the definition — pre-existing).

- **OPTIONAL MATCH WHERE placement: chained/sibling anchor gates (#611) and opaque `size((pattern))`/`EXISTS {}` predicates on the optional variable (#614) no longer drop NULL-extended rows** (#611, #614; #597/#479 family):
  - **#611**: a WHERE on an OPTIONAL clause referencing only a mandatory variable now folds into THAT clause's gating LEFT JOIN ON in two shapes #597 de-scoped: (a) **chained optionals** (`OPTIONAL MATCH (a)-->(b) OPTIONAL MATCH (b)-->(c) WHERE a.x` — the clause's entry connection is the earlier clause's optional `b`, so #597's mandatory-adjacent tagging never landed; the conjunct stayed in the outer WHERE and dropped anchors 3/8 entirely, 18 rows vs the correct 20 incl. `(3,1,NULL)`/`(8,NULL,NULL)`), and (b) **sibling optionals sharing one anchor** (`... WHERE a.x OPTIONAL MATCH ... WHERE a.y` — FilterIntoGraphRel pools per-alias filters onto whichever GraphRel first references the alias, so the second clause's conjunct sat on the FIRST clause's GraphRel, whose gate set didn't contain it, while the second clause's fold was skipped because its own `where_predicate` was None). Three coordinated changes: lowering tags the clause's ENTRY GraphRel (adjacency vs ALL pre-clause-bound aliases; conjunct classification stays mandatory-only); `apply_optional_node_pre_filters` folds off the TAG rather than `where_predicate`, with a clause-precise gating-join finder (LEFT JOINs referencing the anchor, excluding the anchor's own table join, preferring the clause's own edge alias — the old first-match finder picked the earlier clause's node join in the chained shape, and rule-ordering was validated against the FK-edge collapsed-join golden, byte-identical); `collect_graphrel_predicates` drops gate conjuncts against the PLAN-WIDE tag set (wrapper + `collect_optional_anchor_gates`). Deliberately NOT dropped from non-optional GraphRels: a pattern-shaped base MATCH carrying an IDENTICAL conjunct would lose its mandatory row-dropping constraint (FilterIntoGraphRel's covered-aliases dedup leaves one copy serving both roles) — that pattern-base-carrier variant keeps its old main behavior (regression-tested) and is tracked as a follow-up.
  - **#614**: an OPTIONAL clause WHERE conjunct on the optional variable expressed via `size((b)-...)`/`EXISTS { (b)-... }` was left in the outer WHERE (silently dropping the NULL-extended anchors — 9 rows vs the correct 12) because the #479/#552 combined-anchor fold's exactly-one-alias conjunct grouping scanned the opaque pre-rendered SQL for aliases and picked up table/database names (`{social, user_follows_bench, b}` ≠ 1 alias → fold declined). `RenderExpr::PatternCount`/`ExistsSubquery` now carry structurally-recorded `correlated_aliases` (pattern start node; #596's outer-bound EXISTS endpoints), and the fold's grouping uses a new structured-first `collect_correlation_aliases_from_expr` (SQL-scan fallback when unknown). The protection-set path (`collect_referenced_aliases`) deliberately keeps the over-broad scan — narrowing it could under-protect post-WITH CTE-alias correlations. `NOT EXISTS` / `size(...) = 0` variants verified; mixed plain+opaque conjuncts fold together.

  **Adversarial review round 1 found 3 blocking cross-schema/direction regressions in the first cut, all fixed**: (B1) incoming-direction chained (`(b)<-[:FOLLOWS]-(c)`) fail-hard'd — normalization puts the clause's NEW variable in `anchor_connection`/`left_connection`, so the single-sided gating-join search found nothing; the fold now searches the anchor connection first, then the OTHER connection (matching the tagger's pre-bound-entry semantics; live: 25 rows, hand-checked). (B2) chained DENORMALIZED optional silently lost its second LEFT JOIN (Code 47 at execution) — that render path builds joins per-fragment and swallows the fold's loud error; the shared gate helper now excludes any GraphRel whose subtree contains a denormalized standalone-scan Union (`subtree_contains_denormalized_union` — the direct-children check missed the chained nesting), keeping the whole denorm family at its old main-identical placement (ratchet `is_denormalized` 8→9 in plan_builder_helpers.rs: a recursive wrapper over the existing canonical `is_denormalized_union` predicate, not new raw axis branching). (B3) FK-edge sibling optionals had clause 2's gate land on clause 1's collapsed node join (first-candidate fallback); placement is now restricted to CLAUSE-OWNED joins (the GraphRel's edge alias or its connection aliases, loud error otherwise) — which turned out to render the fully CORRECT per-sibling gating (`c ... AND >100`, `d ... AND >200`), better than main's outer-WHERE drop.

  Live-verified (social benchmark, join_use_nulls=1, counts hand-checked against the 10-edge ground truth): chained 18→20, incoming chained (error→) 25; siblings gate-per-clause; #614 size()/EXISTS/NOT-EXISTS 9→12 / 12 / 8; #597 core 12, incoming 12, multi-hop 8 anchors, mixed conjuncts 8 anchors, always-false 8×NULL, base-WHERE and pattern-base-WHERE retention; FK-edge single-clause golden and composite incoming-chained render byte-stable. Corpus golden drift exactly scoped: 1 intended chained-597 relocation + 16 new goldens (both dialects; incl. FK-sibling, incoming-chained, denorm-chained-exclusion locks), zero collateral across ~1,090 entries. Known-remaining (filed as follow-ups on #615): pattern-base-carrier #611 variant; anchor-referencing opaque predicate in an optional WHERE (`WHERE size((a)-...)` — stays outer, drops rows, main-identical); chained denorm anchor gate (deliberately excluded, main-identical).
