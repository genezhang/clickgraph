...
- **Asymmetric denormalized self-loop node-property key sets now fail loud at schema-load, not silent-empty on reverse rows (#1038, #1027 follow-up)**: for a denormalized self-loop node (`from_node == to_node` sharing the edge table), the swap-branch re-key logic (`cte_extraction.rs`) maps each role's property to the opposite role's canonical key; a property present in only ONE direction's map (`from_node_properties` XOR `to_node_properties`) has no opposite-role key, so on reverse (swapped-orientation) rows it silently resolved to empty — the same class of silent-empty #1027 fixed for the symmetric case. The config validator (`validate_denormalized_nodes`, `graph_catalog/config.rs`) previously rejected only an *entirely-missing* direction, not an asymmetric key *set*, leaving such a schema admissible. It now compares the two role maps' key sets for a denorm self-loop node and rejects a mismatch at schema-load with an error naming the offending properties (ground rule #1: fail loud over silent-wrong). Additive, schema-load only — no SQL-emitting code touched (ratchet-neutral, zero golden churn); swept every in-repo denorm schema (`ontime_benchmark`, `ontime_denormalized`, `zeek_merged_test`, `zeek_conn_test`) to confirm none has an asymmetric key set, so no legitimate schema is newly rejected. Locked by `test_denormalized_selfloop_asymmetric_property_keys_rejected_1038` (asymmetric → rejected, error names the offending key) + `test_denormalized_selfloop_symmetric_property_keys_ok_1038` (symmetric, only mapped columns differ by role → still passes; guards against over-rejection).
