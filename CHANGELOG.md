...
- **#504/#529/#461 (coupled-schema / post-WITH legacy-optional family) — R4: two of #529 shape 1's three bugs FIXED, a loud guard added for the third; one of #461 shape 1's three downstream symptom bugs FIXED (a second attempted and reverted after it regressed other tests; a fourth, deeper cause found but not fixed); #504 remains deferred**:
  - **#529 shape 1** — `MATCH (a:IP)-[r:ACCESSED]-(b:IP) WITH a, count(r) AS c RETURN a.ip, c` (plain, non-optional undirected self-edge feeding a `WITH`-aggregate) on `zeek_merged_test.yaml`. Bug 1 (malformed CTE alias, FIXED): `build_union_inner_select` (`sql_generator/emitters/clickhouse/to_sql_query.rs`) reused a self-quoting value expression (e.g. `` r."id.orig_h" ``) as a bare column alias, producing a doubled-quote malformed identifier — fixed via new `split_agg_arg_col`/`agg_arg_alias_key` pure functions used consistently at all four sites that previously mis-parsed this text. Bug 2 (NULL-padding validity gap, FIXED): `table_valid_columns` only consulted `NodeSchema::all_valid_physical_columns()`, so a bare aggregate argument on the relationship's OWN identity (`count(r)` → `r.uid`) was always NULL-padded, making `count(r)` silently return 0 — fixed via a new `RelationshipSchema::all_valid_physical_columns` companion accessor. Bug 3 (UNION branches never alternate `id.orig_h`/`id.resp_h` role for the undirected self-edge — a silent wrong-result bug once bugs 1+2 are fixed) is genuinely deep, cross-cutting planner-level work (traced to two compounding causes: `bidirectional_union.rs`'s per-branch `column_swaps` correction never reaches `WithClause` items, and `find_id_column_for_alias` resolves identity via a role-independent static schema field) — NOT fixed this round. Instead of shipping bugs 1+2 alone (which would trade today's loud crash for tomorrow's silent wrong result — forbidden by this repo's ground rules), added a scoped loud guard (`table_has_role_dependent_denorm_identity` in `render_plan/plan_builder_utils.rs`) that detects the exact precondition — a JOIN-less multi-branch UNION scanning a single physical table whose node identity is role-dependent — and raises a clean `RenderBuildError::UnsupportedFeature` instead. Verified NOT to false-positive on a structurally similar but legitimately-safe multi-hop JOIN-based case (`denorm_with_aggregate_group_by_middle_node_no_null_collapse_465_blocking`, where the JOIN condition — not the SELECT list — encodes the role). The DIRECTED variant of the same pattern (only one branch, guard never engages) is now a genuine, live-verified-passing regression test.
  ...
  Live-verified against ClickHouse (zeek fixture): the #529 shape 1 directed-variant regression test executes and returns row-level counts matching independently hand-computed ground truth from the raw `conn_log` table data. Full test suite (`cargo test`) shows no regressions from the two changes actually kept (the #529 fixes/guard and the #461 `is_optional_pattern` fix) — the reverted `prune_joins_covered_by_cte` attempt is not in the final diff.
