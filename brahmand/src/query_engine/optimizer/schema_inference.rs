...
use crate::{
    open_cypher_parser::ast::{Expression, OperatorApplication, ReturnItem},
    query_engine::types::{GraphSchema, RelationshipSchema, TableData},
};
...
fn get_column_name_from_expression(exp: &Expression) -> Option<String> {
    match exp {
        ...
        Expression::PropertyAccessExp(property_access) => Some(property_access.key.to_string()),
...
fn get_column_name_from_where_conditions(
...
fn get_table_name_from_where_and_return(
    graph_schema: &GraphSchema,
    node_table_data: &TableData,
) -> Option<String> {
    let column_name = if let Some(extracted_column) =
        get_column_name_from_where_conditions(&node_table_data.where_conditions)
    {
        extracted_column
    } else if let Some(extracted_column) =
        get_column_name_from_return_items(&node_table_data.return_items)
    {
        extracted_column
    } else {
        "".to_string()
...
pub fn get_table_names(
    graph_schema: &GraphSchema,
    start_node_table_data: &TableData,
    rel_table_data: &TableData,
    end_node_table_data: &TableData,
) -> Result<(String, String, String), OptimizerError> {
    ...
    // if all present
    if start_node_table_data.table_name.is_some()
        && rel_table_data.table_name.is_some()
        && end_node_table_data.table_name.is_some()
    {
        let start_node_table_name = start_node_table_data
            .table_name
            .ok_or(OptimizerError::MissingNodeLabel)?
            .to_string();
        let end_node_table_name = end_node_table_data
            .table_name
            .ok_or(OptimizerError::MissingNodeLabel)?
            .to_string();
        let rel_table_name = rel_table_data
            .table_name
            .ok_or(OptimizerError::MissingRelationLabel)?
            .to_string();
        return Ok((start_node_table_name, rel_table_name, end_node_table_name));
    }

    // only start node missing
    if start_node_table_data.table_name.is_none()
        && rel_table_data.table_name.is_some()
        && end_node_table_data.table_name.is_some()
    {
        ...
        let rel_table_name = rel_table_data
            .table_name
            .ok_or(OptimizerError::MissingRelationLabel)?; // redundant ok_or
        let rel_schema = graph_schema
            .relationships
            .get(rel_table_name)
            .ok_or(OptimizerError::NoRelationSchemaFound)?;

        let end_table_name = end_node_table_data
            .table_name
            .ok_or(OptimizerError::MissingNodeLabel)?;
    ...
    if start_node_table_data.table_name.is_some()
        && rel_table_data.table_name.is_some()
        && end_node_table_data.table_name.is_none()
    {
        // check relation table name and infer the node
        let rel_table_name = rel_table_data
            .table_name
        ...
        let start_table_name = start_node_table_data
            .table_name
            .ok_or(OptimizerError::MissingNodeLabel)?;
    ...
    if start_node_table_data.table_name.is_some()
        && rel_table_data.table_name.is_none()
        && end_node_table_data.table_name.is_some()
    {
        let start_table_name = start_node_table_data
            .table_name
        ...
        for (_, relation_schema) in graph_schema.relationships.iter() {
            if (relation_schema.from_node == start_table_name
                && relation_schema.to_node == end_table_name)
                || (relation_schema.from_node == end_table_name
                    && relation_schema.to_node == start_table_name)
            {
                return Ok((
                    start_table_name.to_string(),
        ...
        && end_node_table_data.table_name.is_none()
    {
        ...
        let extracted_start_node_table_result =
            get_table_name_from_where_and_return(graph_schema, start_node_table_data);
        let extracted_end_node_table_result =
            get_table_name_from_where_and_return(graph_schema, end_node_table_data);
        ...
        if extracted_start_node_table_result.is_some() {
            ...
            let start_table_name = &graph_schema
                .nodes
                .get(&relation_schema.from_node)
                .ok_or(OptimizerError::NoNodeSchemaFound)?
                .table_name;
            let end_table_name = &graph_schema
                .nodes
                .get(&relation_schema.to_node)
                .ok_or(OptimizerError::NoNodeSchemaFound)?
                .table_name;
        ...
        && end_node_table_data.table_name.is_none()
    {
        ...
        if relations_found.len() > 1 && extracted_end_node_table_result.is_some() {
            ...
            for relation_schema in relations_found {
                ...
                if (relation_schema.from_node == start_table_name
                    && relation_schema.to_node == extracted_end_node_table_name)
                    || relation_schema.to_node == start_table_name
                        && relation_schema.from_node == extracted_end_node_table_name
                {
        ...
        && end_node_table_data.table_name.is_some()
    {
        ...
        if relations_found.len() > 1 && extracted_start_node_table_result.is_some() {
            ...
            for relation_schema in relations_found {
                ...
                if (relation_schema.from_node == end_table_name
                    && relation_schema.to_node == extracted_start_node_table_name)
                    || relation_schema.to_node == end_table_name
                        && relation_schema.from_node == extracted_start_node_table_name
                {
        ...
        && end_node_table_data.table_name.is_none()
    {
        ...
        else if extracted_start_node_table_result.is_some()
            && extracted_end_node_table_result.is_none()
        {
            let start_table_name = extracted_start_node_table_result.unwrap();
            for (_, relation_schema) in graph_schema.relationships.iter() {
                if relation_schema.from_node == start_table_name {
        ...
        else if extracted_start_node_table_result.is_none()
            && extracted_end_node_table_result.is_some()
        {
            let end_table_name = extracted_end_node_table_result.unwrap();
            for (_, relation_schema) in graph_schema.relationships.iter() {
                if relation_schema.from_node == end_table_name {
...
mod tests {
    ...
    use crate::{
        open_cypher_parser::ast::{FunctionCall, Literal, Operator, PropertyAccess},
        query_engine::types::{NodeIdSchema, NodeSchema},
    };
    ...
    fn test_get_column_name_from_property_access() {
        ...
        assert_eq!(
            get_column_name_from_expression(&expr),
            Some("col1".to_string())
        );
    }
    ...
    fn test_get_column_name_from_operator_application() {
        ...
        assert_eq!(
            get_column_name_from_expression(&nested),
            Some("col2".to_string())
        );
    }
    ...
    fn test_get_column_name_from_function_call() {
        ...
        assert_eq!(
            get_column_name_from_expression(&func),
            Some("col3".to_string())
        );
    }
    ...
    fn test_get_column_name_from_return_items() {
        ...
        assert_eq!(
            get_column_name_from_return_items(&items),
            Some("col4".to_string())
        );

        let empty = vec![ReturnItem {
    ...
    fn test_get_column_name_from_where_conditions() {
        let conds = vec![OperatorApplication {
        ...
        assert_eq!(
            get_column_name_from_where_conditions(&conds),
            Some("col5".to_string())
        );

        let no_props = vec![OperatorApplication {
            operator: Operator::Not,
            operands: vec![Expression::Literal(Literal::Boolean(false))],
        }];
    ...
    fn where_conditions_match() {
        ...
        assert_eq!(
            get_table_name_from_where_and_return(&schema, &td),
            Some("car_table".to_string())
        );
    }
