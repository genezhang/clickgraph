use crate::{
    ...
    query_engine::types::{
        EntityProperties, GraphSchema, GraphSchemaElement, NodeIdSchema, NodeSchema,
        RelationshipSchema,
    },
};
...
fn get_default_value(default_value_expr: &Expression) -> Result<String, ChQueryGeneratorError> {
    match default_value_expr {
        Expression::Literal(literal) => Ok(get_literal_to_string(literal)),
        _ => {
            // throw error
            Err(ChQueryGeneratorError::UnsupportedDefaultValue)
        }
    }
}
...
fn get_node_props(
...
) -> Result<EntityProperties, ChQueryGeneratorError> {
    ...
    if primary_keys.is_empty() {
        return Err(ChQueryGeneratorError::MissingPrimaryKey);
    }

    if node_id.is_empty() {
        return Err(ChQueryGeneratorError::MissingNodeId);
    }

    if node_id.len() > 1 {
        return Err(ChQueryGeneratorError::MultipleNodeIds);
    }
    ...
    let node_id_column_schema = columns
        ...
        .ok_or(ChQueryGeneratorError::InvalidNodeId)?;

    if !["Int64", "UInt64"].contains(&node_id_column_schema.column_dtype) {
        return Err(ChQueryGeneratorError::InvalidNodeIdDType);
    }
...
fn generate_create_node_table_query(
    ...
    let columns_vec: Vec<String> = create_node_table_clause
        .table_schema
        .iter()
        .map(|column_schema| -> Result<String, ChQueryGeneratorError> {
        ...
        .collect::<Result<Vec<String>, ChQueryGeneratorError>>()?;
...
fn generate_create_rel_table_query(
    ...
    let from_table_schema = current_graph_schema
        .nodes
        .get(from_node)
        .ok_or(ChQueryGeneratorError::UnknownFromTableInRel)?;
    let to_table_schema = current_graph_schema
        .nodes
        .get(to_node)
        .ok_or(ChQueryGeneratorError::UnknownToTableInRel)?;
...
pub fn generate_query(
    ...
    // throw error
    Err(ChQueryGeneratorError::UnsupportedDDLQuery)
}
...
mod tests {
    ...
    fn err_missing_primary_key() {
        ...
        assert!(matches!(err, ChQueryGeneratorError::MissingPrimaryKey));
    }
    ...
    fn err_missing_node_id() {
        ...
        let err = get_node_props(props, &cols).unwrap_err();
        assert!(matches!(err, ChQueryGeneratorError::MissingNodeId));
    }
    ...
    fn err_multiple_node_ids() {
        ...
        let err = get_node_props(props, &cols).unwrap_err();
        assert!(matches!(err, ChQueryGeneratorError::MultipleNodeIds));
    }
    ...
    fn err_invalid_node_id() {
        ...
        let err = get_node_props(props, &cols).unwrap_err();
        assert!(matches!(err, ChQueryGeneratorError::InvalidNodeId));
    }
    ...
    fn err_invalid_node_id_dtype() {
        ...
        let err = get_node_props(props, &cols).unwrap_err();
        assert!(matches!(err, ChQueryGeneratorError::InvalidNodeIdDType));
    }
    ...
    fn error_unknown_from() {
        ...
        assert!(matches!(err, ChQueryGeneratorError::UnknownFromTableInRel));
    }
    ...
    fn error_unknown_to_concrete() {
        ...
        let err = generate_create_rel_table_query(clause, &make_schema()).unwrap_err();
        assert!(matches!(err, ChQueryGeneratorError::UnknownToTableInRel));
    }
    ...
    fn generate_query_unsupported() {
        ...
        assert!(matches!(err, ChQueryGeneratorError::UnsupportedDDLQuery));
    }
}
