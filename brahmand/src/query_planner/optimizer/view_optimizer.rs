...
impl ViewOptimizer {
    ...
    fn simplify_filter_expression(&self, filter: &LogicalExpr) -> LogicalExpr {
        match filter {
            ...
            LogicalExpr::OperatorApplicationExp(op) => {
                let optimized_operands = op.operands.iter()
    ...
    fn flatten_and_operands(&self, operands: &[LogicalExpr], result: &mut Vec<LogicalExpr>) {
        for operand in operands {
            match operand {
                LogicalExpr::OperatorApplicationExp(nested_op) if nested_op.operator == Operator::And => {
        ...
        let transformed_plan = match logical_plan.as_ref() {
            LogicalPlan::ViewScan(view_scan) => {
                ...
                match optimized_scan {
                    ...
                    Transformed::No(scan) => {
                        Transformed::No(Arc::new(LogicalPlan::ViewScan(scan)))
                    },
                }
            },
