#!/bin/bash
# Load LDBC SNB data via Docker with proper escaping
# Usage: ./load_data_docker_v2.sh sf10
...
echo "=========================================="
echo "LDBC SNB Data Loader (Docker v2)"
echo "=========================================="
echo "Scale Factor: $SCALE_FACTOR"
echo "=========================================="

# Helper function to load CSV with custom settings
load_table() {
    local table=$1
    local subdir=$2
    ...
    clickhouse-client --user="$USER" --password="$PASSWORD" --database="$DATABASE" \
        --query="TRUNCATE TABLE IF EXISTS $table" 2>/dev/null
    
    # Load each CSV file
    for csv_file in $csv_files; do
        # Use input_format_csv_allow_variable_number_of_columns to handle weird delimiters
        # Also set allow_null_or_missing_columns for optional fields
        clickhouse-client --user="$USER" --password="$PASSWORD" --database="$DATABASE" \
