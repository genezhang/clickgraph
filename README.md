...
- You need **iterative whole-graph algorithms** (PageRank, community detection, betweenness) and the graph fits in memory — that's Neo4j GDS / cuGraph / TigerGraph territory, on a snapshot. ClickGraph deliberately keeps these out of scope rather than mis-rendering them. (A family of these algorithms is iterated `JOIN`+`GROUP BY` over a semiring and is a natural roadmap direction inside SQL — but it is not shipped today.)
