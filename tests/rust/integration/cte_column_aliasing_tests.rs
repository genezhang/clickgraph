...
fn create_test_schema() -> GraphSchema {
    ...
    property_mappings.insert("user_id".to_string(), PropertyValue::Column("user_id".to_string()));
    property_mappings.insert("name".to_string(), PropertyValue::Column("full_name".to_string()));
    property_mappings.insert("email".to_string(), PropertyValue::Column("email_address".to_string()));

    let user_schema = NodeSchema {
        database: "test".to_string(),
        table_name: "users".to_string(),
        column_names: vec!["user_id".to_string(), "full_name".to_string(), "email_address".to_string()],
...
fn test_cte_column_aliasing_underscore_convention() {
    ...
    assert!(sql.contains(" AS a_") || sql.contains(" AS \"a_"),
