...
with_a_c_cte_1 AS (SELECT 
      ...
      (SELECT count(*) FROM (SELECT explode(a_friends.`friends`) AS x) WHERE x IN (SELECT __r0.follower_id FROM test_integration.user_follows_test AS __r0 UNION SELECT __r0.followed_id FROM test_integration.user_follows_test AS __r0)) AS `c`
