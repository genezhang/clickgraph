# ClickGraph + Neo4j Browser Performance Testing

**Internal performance testing suite with Neo4j Browser visualization using real benchmark data**

This is the **internal performance testing setup** for ClickGraph with large-scale benchmark data (954.9M rows).

For **end-user quick start with Neo4j Browser**, see `demos/neo4j-browser/` instead.

Connect the Neo4j 5.15 community edition to ClickGraph server via Bolt protocol for interactive graph exploration at scale.
...
```bash
cd /home/gene/clickgraph/tests/neo4j-browser
bash setup.sh
```
