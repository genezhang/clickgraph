...
Schema: social_demo (User, Post, FOLLOWS, AUTHORED, LIKED)
"""
...
class TestBrowserDirectionValidation:
    ...
    def test_invalid_direction_post_to_user(self):
        """Post→User for LIKED should return empty (no such relationship)"""
        response = query("MATCH (p:Post)-[r:LIKED]->(u:User) RETURN count(*) as count")
        assert response.status_code == 200
        data = response.json()
        # Should return 0 (or empty array if aggregate bug unfixed)
...
class TestBrowserErrorCases:
    ...
    def test_nonexistent_label(self):
        ...
        assert response.status_code == 500  # Should error gracefully
    ...
    def test_nonexistent_relationship_type(self):
        ...
        # Might return empty or error - both acceptable
...
class TestBrowserSQLGeneration:
    ...
    def test_sql_for_unlabeled_relationship_click(self):
        ...
        # SQL should NOT try to join users.post_id (doesn't exist)
        assert "users AS t" in sql.lower()
        assert "posts AS t" in sql.lower()
        
        # Check for invalid patterns that cause the bug
        # Should NOT have: users JOIN users via post_id
        lines = sql.lower().split('\n')
        for i, line in enumerate(lines):
            if 'users as t' in line and i+2 < len(lines):
                # Check next few lines for the join
                next_lines = ' '.join(lines[i:i+3])
                # Should not join users to users via post_id
                if 'join social.users' in next_lines:
                    assert 'post_id' not in next_lines, "Invalid join: users.post_id doesn't exist!"
